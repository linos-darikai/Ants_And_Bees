            ...
            if self.ant!= None:
            	if self.ant.container== True:
            		if self.ant.can_contain(insect)==True:
            			self.ant.ant = insects
            	if insect.container == True:
            	    if insect.can_contain(self.ant)== True:
            	        insect.ant = self.ant
            	        self.ant = insect
            else:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive , min_range = None , max_range= None):
...
class HungryAnt(Ant):
    ...
    def eat_bee(self, bee):
        if self.eating>0:
        	self.eatimg = self.eating - 1
        elif self.eating==0:
            bee_armor=0
            bee.place.remove_insect(bee)
            self.eating= self.time_to_eat	
...
def apply_effect(effect, bee, duration):
    ...
    original_bee = bee.action
    def status(colony):
        nonlocal duration
        if duration > 0:
            duration -= 1 
            return effect(original_bee)(colony)
        else: 
            return original_bee(colony)
    bee.action =status
